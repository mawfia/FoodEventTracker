...
var foodModule = function(id, operation){
	...
	switch(operation){
		...
		case 'Create2': foodReq('POST', URL, 'application/json', foodData($food), buildFoodTable); $food = null; foodReq('GET', URL, null, null, buildFoodTable); break;
...
var foodForm = function($food, title){
	...
	//var $name = $('<form:label path="name" >Name:</form:label><form:input path="name" class="form-control" type="text" placeholder="happy"/><form:errors path="name" />');
	//var $description = $('<form:label path="description" >Description:</form:label><form:input path="description" class="form-control" type="text"/><form:errors path="description" />');
	//var $input = $('<input type="submit" value="Submit"  class="btn btn-default"></input>');
	...
	var $weight = $('<label>Weight:</label><input name="weight" class="form-control" type="number" placeholder="In Pounds" id="weight">');
	...
	var $button = $('<button type="submit" class="btn btn-primary" value="5" id="Create2">Submit</button>'); BtnEvnt($button);
